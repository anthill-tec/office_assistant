...
- **The fakes-only gap is now closed by a real-server tier (2026-07-29).** The in-process fakes encode *our* assumptions of each provider — the class that let the round-1 empty-Fastmail-draft bug pass green. [DN-mail-e2e-emulator-testing.md](DN-mail-e2e-emulator-testing.md) specifies and delivers the fix: a LOCAL, pre-release `@pytest.mark.e2e` tier driving the real verbs against a Dockerized Stalwart emulator (the `identityId`, TLS-verification and mailbox-quoting items above are three defects it caught that the fakes could not). **Residual, accepted:** XOAUTH2 SMTP stays fake-covered (it needs an OAuth token endpoint), Gmail `X-GM-RAW` *semantics* remain real-Gmail-only, and an emulator profile is a strong proxy rather than proof against real Fastmail/Gmail — a real-account spot-check stays an optional manual pre-ship check.
