...
- **JMAP send — `EmailSubmission/set` + `onSuccessUpdateEmail`** clears `$draft` and moves the message to the `sent`-role mailbox. The submission carries the **`identityId` RFC 8621 §7.1 requires**, resolved first by a batched `Identity/get` (`ids: null`) + `Email/get` of the draft's own `From`, then matched to the identity authorized for that From (exact address, else a `*@domain` wildcard, else the first identity) — an unmatched identity is refused `forbiddenFrom` §7.5, and `mail-auth --alias` makes sending as an alias a first-class flow, so "the first identity" is not sufficient. Only the `Identity/get` outcome is fatal; an erroring `Email/get` degrades to the first identity. Fastmail's leniency (it assigns a default identity) is why the fakes never caught the omission — a spec-strict server rejects it. The Sent-role lookup is **non-fatal**: a method-level *or* transport-level failure (`HTTPError`/`OSError` from a 4xx/5xx, `ValueError` from a non-JSON 2xx body) skips only the file-to-Sent move — a submission needs no mailbox, so the lookup never blocks a send.
- **IMAP/SMTP send — a partial refusal is a failure, then safe-gated file-to-Sent.** `smtplib.sendmail` raises only when *every* `RCPT TO` was refused and otherwise returns the per-recipient refusal dict quietly, so a submission accepted for some recipients would read as a clean send. That dict is read: a non-empty one raises structurally, naming each refused address, **before** any bookkeeping — the draft stays in Drafts to fix and re-send, and no FK-linked row records the correspondence as delivered. Only once every recipient was accepted are the sent bytes `APPEND`ed `\Seen` to the `\Sent` special-use mailbox (skipped for Gmail, which files its own), and then the Drafts copy retired (`-FLAGS \Draft`, `+FLAGS \Deleted`, UID `EXPUNGE`) **only after a confirmed Sent copy exists** — a tagged-`OK` `APPEND`, or a provider that self-files. No `\Sent` mailbox, a refused `APPEND`/`STORE`, or any raised bookkeeping failure leaves the draft untouched, so a sent message can never end up in neither folder. That bookkeeping runs only after a fully accepted delivery — none of it can fail a sent message.
- **Folder resolution — RFC 6154 special-use via `LIST`** for **both** Sent and Drafts (quoted or bare-atom names; tagged status checked; a non-answer never cached), with fallbacks (`[Gmail]/Drafts`, `Draft`). A resolved name is **quoted for the wire when it contains a space** (`Sent Items`, `Draft Items`) on every `APPEND` *and* `SELECT` — `imaplib` forwards the argument verbatim, so an unquoted spaced name is parsed as its first word; atoms (`Sent`, `[Gmail]/Drafts`) pass through byte-identical. IMAP `create_draft` raises structurally on a refused `APPEND` (parity with the JMAP `notCreated` handling), never a `drafted` status carrying the server's error text as the draft id.
- **TLS is VERIFIED on both IMAP and SMTP.** `imaplib.IMAP4_SSL` and `smtplib.SMTP.starttls()` are each handed an explicit `ssl.create_default_context()` (CERT_REQUIRED + hostname check) — without one the stdlib default is `CERT_NONE`, i.e. an unauthenticated channel over which a MITM could harvest the account's app-password. The single per-account opt-out is `endpoint.tls_verify: false`, which exists **only** for the local emulator's self-signed cert (see [DN-mail-e2e-emulator-testing.md](DN-mail-e2e-emulator-testing.md)); absent the key the verifying default holds, and `voa doctor` surfaces any account that turned it off.
...
- **The fakes-only gap is now closed by a real-server tier (2026-07-29).** The in-process fakes encode *our* assumptions of each provider — the class that let the round-1 empty-Fastmail-draft bug pass green. [DN-mail-e2e-emulator-testing.md](DN-mail-e2e-emulator-testing.md) specifies and delivers the fix: a LOCAL, pre-release `@pytest.mark.e2e` tier driving the real verbs against Dockerized emulators (the `identityId`, TLS-verification and mailbox-quoting items above are three defects it caught that the fakes could not). What that tier reproduces and what it still cannot — including the accepted residual — is owned by that DN's **Fidelity** section; do not restate it here.
