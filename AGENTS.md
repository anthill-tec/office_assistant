...
3. **On the release branch, BEFORE `finish`, run the validation set IN THIS ORDER and confirm each is GREEN.** The order is not cosmetic: the no-mistakes reconcile **rewrites local history**, so it must finish before anything is pushed or dispatched.
   ...
   3. the **release gate**, the **full test suite**, and the **local E2E mail smoke run** (order among these three is free):
      ...
      - **local E2E mail smoke run** — `make e2e` (`.venv/bin/python -m pytest -m e2e tests/e2e -v`), after a one-time `make e2e-install` (the `[e2e]` extra). It drives the real `voa` mail verbs (and the real `GmailXoauth2Adapter`) against **two** throwaway Dockerized emulators — **Stalwart** for the JMAP + PLAIN-password profiles and **Dovecot 2.3** (plus a token-introspection stub + SMTP sink) for the XOAUTH2 profile — so it needs **Docker running locally** and the **`openssl` CLI** (the Dovecot fixture generates a throwaway self-signed cert; without it that fixture skips). This is a **separate** pass, not part of the bullet above: `pyproject.toml` pins `addopts = -m 'not e2e'`, so `pytest tests/ -q` never collects the tier. It is **LOCAL-ONLY and never in CI** — required green before `finish`, and after any change to the mail subsystem. Design of record: [`docs/research/DN-mail-e2e-emulator-testing.md`](docs/research/DN-mail-e2e-emulator-testing.md).
