...
0. **Per-CR gate (before you ever get to a release).** **`no-mistakes` runs on each FEATURE branch, BEFORE `git flow feature finish`** — that is the run that actually reviews the CR's code. By release time every CR is already merged into `develop`, so the release-branch run below only validates the **release-branch delta** (typically docs/version bookkeeping). A release-branch run is **not** a substitute for the per-CR runs.
...
3. **On the release branch, BEFORE `finish`, run the full validation set and confirm each is GREEN:**
   ...
   - a **TestPyPI dry-run** — push the release branch (`git push -u origin release/X.Y.Z`), then dispatch CI **pinned to that branch**: `gh workflow run ci.yml --ref release/X.Y.Z`. Confirm `test-publish` uploads cleanly to TestPyPI **before** any production publish. **Never dispatch against `main`** — the production `publish` job is gated on `github.ref == 'refs/heads/main'` with no event-type guard, so a dispatch on `main` runs the real PyPI publish, not a dry-run.
